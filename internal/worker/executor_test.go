...
func (p *testPublisher) Publish(jobID apitypes.JobID, line string) {
...
func TestExecutor_HostExecution(t *testing.T) {
	...
	pub := &testPublisher{}
	exec := NewExecutor(nil, nil, nil, toolchain.NewRegistry(), pub, tempDir)
	...
	// Verify persistent workspace was created and not deleted
	...
	// Verify second command runs in the same persistent workspace
	...
	// Test 3: Multiple projects owned by same user do not collide
	...
	// Verify project B does NOT have project A's file
